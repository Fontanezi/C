...
typedef struct
{
    REGISTRO A[MAX + 1]; // EXTRA SPACE FOR A SENTINEL
    int noElem;
} LISTA;
...
// Searches for a key in the list using a sentinel
int listSearchSentinel(int key, LISTA *l)
{
    int i = 0;
    l->A[l->noElem].chave = key;
    while (l->A[i].chave != key)
    {
        i++;
    }
    if (i == l->noElem)
...
{
    int pos = listSearchSentinel(key, l);
    if (pos == -1)
    {
        return 0;
    }
