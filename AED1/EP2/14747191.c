...
/**								                                    **/
/**   João Fontanezi		  14747191          TURMA 94            **/
/**								                                    **/
/*********************************************************************/
...
/* Função recursiva para buscar uma palavra na trie */
int buscarPalavraAux(PONT atual, char* palavra, int n, int pos) {
    if (!atual) return 0; // Nó inexistente, palavra não encontrada
    if (pos == n) return atual->contador; // Última letra da palavra, retorna contador
    int indice = palavra[pos] - VALOR_a;
    if (!atual->filhos || !atual->filhos[indice]) return 0; // Letra não encontrada
    return buscarPalavraAux(atual->filhos[indice], palavra, n, pos + 1); // Próximo nível
}
...
PONT atual = raiz;
    ...
    atual->contador++; // Incrementa o contador da última letra
}
...
void excluirTodas(PONT raiz, char* palavra, int n){
    PONT atual = raiz;
    PONT stack[n]; // Pilha para rastrear os nós
    int indices[n]; // Armazena os índices dos caracteres
    int topo = 0;

    // Navega até a última letra da palavra
    for (int i = 0; i < n; i++) {
        if (!atual->filhos) return; // Palavra não existe
        int indice = palavra[i] - VALOR_a;
        if (!atual->filhos[indice]) return; // Letra não encontrada
        stack[topo] = atual; // Empilha o nó atual
        indices[topo++] = indice;
        atual = atual->filhos[indice];
    }

    atual->contador = 0; // Zera o contador da última letra

    // Remove os nós desnecessários na volta
    while (topo > 0) {
        PONT pai = stack[--topo];
        int indice = indices[topo];

        // Exclui o nó se ele não tiver filhos e o contador for zero
        if (pai->filhos[indice]->contador == 0) {
            bool hasChildren = false;
            for (int i = 0; i < LETRAS; i++) {
                if (pai->filhos[indice]->filhos && pai->filhos[indice]->filhos[i]) {
                    hasChildren = true;
                    break;
                }
            }
            ...
            if (!hasChildren) {
                free(pai->filhos[indice]->filhos);
                pai->filhos[indice]->filhos = NULL;
                free(pai->filhos[indice]);
                pai->filhos[indice] = NULL;
            }
        }

        // Verifica se o pai ainda tem outros filhos
        bool paiTemFilhos = false;
        for (int i = 0; i < LETRAS; i++) {
            if (pai->filhos && pai->filhos[i]) {
                paiTemFilhos = true;
                break;
            }
        }

        // Se o pai não tiver filhos nem contador, libera o arranjo de filhos
        if (!paiTemFilhos && pai->contador == 0) {
            free(pai->filhos);
            pai->filhos = NULL;
        }
    }
...
void excluir(PONT raiz, char* palavra, int n){
    ...
    // Se contador é maior que 1, apenas decrementa
    if (atual->contador > 1) {
        atual->contador--;
    } else {
        // Caso contrário, exclui todas as cópias
